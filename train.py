from graph_data import GoTermDataset, collate_fn, SwissDataSet
...
def train(config, task, suffix):
    ...
    #pos_weights = torch.tensor(np.stack([pos_weights, np.ones_like(pos_weights)]).T).float()
    valid_set = GoTermDataset("val", task, config.AF2model, config.SWmodel)
    ...
    SW_train_set = SwissDataSet(task, config.device)
    SWtrain_loader = DataLoader(SW_train_set, batch_size=config.batch_size, shuffle=True, collate_fn=collate_fn)
    ...
    mlsm_loss = torch.nn.MultiLabelSoftMarginLoss().to(config.device)
    ce_loss = torch.nn.CrossEntropyLoss()
    #criterion = NT_Xent(config.batch_size, 0.1, 1)
    ...
    #not_all_0idx = np.int16(set(np.where(y_true_all == 1.)[0].tolist()))
    #all_0idx = np.setdiff1d(np.arange(3321), not_all_0idx)
    #y_true_all = y_true_all[not_all_0idx,:]
    ...
    for ith_epoch in range(config.max_epochs):
        ...
        if config.SWmodel == True:
            #sw_indx = np.random.choice(7)
            #if True:
                #del SW_train_set
                #del SWtrain_loader
                #SW_train_set = SwissDataSet(task, config.device)
            ...
            for idx_batch, batch in enumerate(SWtrain_loader):
                # with torch.autograd.set_detect_anomaly(True):
                model.train()
                #optimizer.zero_grad()
                ...
                log(f"SW:{idx_batch}/{ith_epoch} train_epoch ||| Loss: {round(float(loss),3)}")
                train_loss.append(loss.clone().detach().cpu().numpy())
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
if __name__ == "__main__":
    ...
    p.add_argument('--SWmodel', default=False, type=str2bool, help='whether to use Swiss-model for training')
    args = p.parse_args()
    config = get_config()
